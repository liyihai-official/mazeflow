...
class Generator2D():
    """ Generator of 2D mazes
    Inputs:
        - grid_shape: (x, y), the height and width of 2d mazes.
        - start: (x, y), the coordinate of start
        - goals: [(x0, y0), (x1, y1), ...], The list of coordinates of goals.
    """
    def __init__(self, grid_shape, start, goals):
    ...
    def generate_walls_array(self, threshold=0.5):
        '''Transform the random array in to walls and no-walls nodes with 
            input threshold
        
        Input: 
            - threshold: A value helped generating walls. 
            Values of noise array will be set as wall (1) as long as it is 
    ...
    def save(self, filename=None, save=False):
        """Save the model as contents by default. Also, it can store in the 
        specified file.

        Inputs:
            - filename: string of filename you want to store. Default as None.
            - save: Save the mazes into file or not. Default as False.
        """
        contents = []
    ...
    def print(self, grid=None):
        """Print the internal grid or input grid"""
        if not grid:
            grid = self.save()
